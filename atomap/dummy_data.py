...
def get_fantasite():
    ...
    test_data.add_atom_list(
            xA1, yA1, sigma_x=3, sigma_y=3, amplitude=10)
    dx = 1
    for i in range(45,90,18):
        ...
        test_data.add_atom_list(
            xA2, yA2, sigma_x=3, sigma_y=3, amplitude=10)
        test_data.add_atom_list(
            xA3, yA3, sigma_x=3, sigma_y=3, amplitude=10)
    for i in range(99,135,18):
        xA4, xA5 = xA0[i:i+9], xA0[i+9:i+18]
        yA4 = yA0[i:i+9] + dx
        yA5 = yA0[i+9:i+18] - dx
        test_data.add_atom_list(
            xA4, yA4, sigma_x=3, sigma_y=3, amplitude=10)
        test_data.add_atom_list(
            xA5, yA5, sigma_x=3, sigma_y=3, amplitude=10)
    xB1, yB1 = np.mgrid[10:295:20, 27:290:34]
    test_data.add_atom_list(
            xB1.flatten()[:40], yB1.flatten()[:40], sigma_x=3, sigma_y=3, amplitude=20)
    ...
    for i, x in enumerate(xB2):
        ...
        test_data.add_atom(x, yB2[i], sigma_x=3, sigma_y=sigma_y, amplitude=20, rotation=0.39)
    test_data.add_image_noise(mu=0, sigma=0.01)
    return test_data.signal
