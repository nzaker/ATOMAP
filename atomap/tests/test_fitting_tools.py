import unittest
from atomap.fitting_tools import ODR_linear_fitter, linear_fit_func, get_shortest_distance_point_to_line
import numpy as np
...
class test_ODR_fitter(unittest.TestCase):
    ...
    def test_vertical(self):
        ...
        x = np.full_like(y, 5)
        beta = ODR_linear_fitter(x, y)
        y_vector = [0,linear_fit_func(beta, 5)]
        x_vector = [1,0]
...
class test_find_distance_point_line(unittest.TestCase):
    ...
    def test_simple(self):
        x_list, y_list = [0], [0]
        line = [0, 1]
        d = get_shortest_distance_point_to_line(x_list,y_list,line)
