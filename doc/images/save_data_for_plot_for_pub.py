import atomap.api as am
import numpy as np
import hyperspy.api as hs
# First we will analyse and save the structural data of interest
# Here, we use the fantasite dummy data.
atom_lattice = am.dummy_data.get_fantasite_atom_lattice()

sublattice_A = atom_lattice.sublattice_list[0]
np.savez('sublattice_A.npz', x=sublattice_A.x_position, y=sublattice_A.y_position, e=sublattice_A.ellipticity)

sublattice_B = atom_lattice.sublattice_list[1]
np.savez('sublattice_B.npz', x=sublattice_B.x_position, y=sublattice_B.y_position, e=sublattice_B.ellipticity)

sublattice_A.construct_zone_axes()
zone = sublattice_A.zones_axis_average_distances[0]
s_dd = sublattice_A.get_atom_distance_difference_map([zone])
s_dd.save('distance_difference_map.hdf5', overwrite=True)
#s_dd = s_dd.isig[40.:460.,40.:460.]

im = atom_lattice.image0
s_adf = hs.signals.Signal2D(im)
s_adf.save('ADF_image.hdf5', overwrite=True)
#s_adf = s_adf.isig[40.:460.,40.:460.]

z1 = sublattice_A.zones_axis_average_distances[0]
z2 = sublattice_A.zones_axis_average_distances[1]
plane = sublattice_A.atom_planes_by_zone_vector[z2][23]
s_dd_line = sublattice_B.get_atom_distance_difference_line_profile(z1,plane)
s_dd_line.save('dd_line.hdf5', overwrite=True)
